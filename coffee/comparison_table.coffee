###
# The goal here is to get a string that evaluates to the desired value.
# This string is set to object.asString
# For complicated cases which can't be passed in JSON the value
# can be passed as a string wrapped in backticks which is evaluated here.
# Example: "`parseFloat("nan")`" means NaN.
# The values which are strings wrapped in backticks (`) are evaluatedbefore being compared.
###
isString = (obj) ->
...
class ForComparison
    ...
    testResults: (fc2, comparator) ->
        evalStr = ""
        comparator = "===" unless comparator?
        t = this
        comparator.split(/&&/).forEach (c) ->
            nawias = false
...
###
# The values which are strings wrapped in backticks (`) are evaluated before being compared
###
...
testRepr = (what, shouldBe) ->
    ...
    throw new Error("Value is not being represented correctly.") if fc.toString() isnt shouldBe
...
testEquality = (tf, item, comparator) ->
    ...
    results = fc1.testResults(fc1, comparator)
    throw new Error("Condition (" + results[0] + ") should be " + tf) if results[1] isnt tf

testEquality true is true, "`true`", "=="
testEquality [ [] ] is [ [] ], "`[[]]`", "=="
testEquality [] is [], "`[]`", "=="
...
@buildComparisonTable = (values, comparator) ->
    ...
    for comp in comps
        $el = if supportsCanvas then rotateText(comp.asString) else $("<span>",
            class: "rotate"
            text: comp.asString
        )
...
rotateText = (txt, cHeight) ->
    cHeight = 80 unless cHeight?
